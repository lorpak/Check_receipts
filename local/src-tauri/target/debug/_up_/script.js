...
function toggleFilters(disable) {
    ...
    if (startDateInput) {
        startDateInput.disabled = disable;
    }
    if (endDateInput) {
        endDateInput.disabled = disable;
    }
...
function selectSource(evt) {
    const isFile = evt && evt.shiftKey;
    if (isFile) {
        selectFolder('Выберите XML-файл для точечной обработки', 'sourceFolderPath', 'file');
    } else {
        selectFolder('Выберите одну или несколько папок (Ctrl/Shift)', 'sourceFolderPath', 'multi_folder');
    }
}
